...
public function addProducts(Request $request)
{
    ...
    try {
        ...
        // Add each new product to the order
        foreach ($data['products'] as $product) {
            DB::table('order_to_staff_transaction')->insert([
                ...
                'orderTotalProductPrice' => $product['totalPrice'],
                'orderTotalProductTax' => $product['totalPrice'] * 0.12, // Assuming 12% tax
                'orderProductStatus' => 'For Payment',
                'orderNotes' => $product['notes'] ?? null,
        ...
        return response()->json([
            'success' => false,
            'message' => 'Failed to add products to order: ' . $e->getMessage()
        ], 500);
    }
}
...
public function getMenuProducts()
{
    try {
        $products = DB::table('menu_products')
            ...
            ->select('menuID', 'menuName', 'menuCategory', 'menuSubcategory', 'menuPrice')
